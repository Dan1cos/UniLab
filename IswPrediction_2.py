...
def secondSt(df):
    ...
    def remove_unique(page_html_text):
        ...
        page_html_text = page_html_text.replace(
        ...
        page_html_text = page_html_text.replace(
            "10 U.S. Code § 885 - Art. 85. Desertion | U.S. Code | US Law | LII / Legal Information Institute (cornell.edu)",
            "")
        page_html_text = page_html_text.replace("https//denis-pushilin  dot ru/doc/ukazy/Ukaz_15_23012023 dot pdf", "")
        page_html_text = page_html_text.replace(
            "Who is Russia's 'butcher of Syria,' now leading the invasion of Ukraine? : NPR Russian General Officer Guide ISW May 11 .pdf (understandingwar.org)",
            "")
        page_html_text = page_html_text.replace(
    ...
    def remove_months_pm_am_th(data):
        data = re.sub(r"(january|february|march|april|may|june|july|august|september|october|november|december)", "",
                      data)
